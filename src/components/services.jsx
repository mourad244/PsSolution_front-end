...
class Services extends Component {
	...
	handleDelete = async (service) => {
		...
		try {
			...
			if (ex.response && ex.response.status === 404) toast.error('service déja supprimé');
	...
	getPagedData = () => {
		const { pageSize, currentPage, sortColumn, selectedCategorie, searchQuery, services: allServices } = this.state;
		let filtered = allServices;
		if (searchQuery)
			filtered = allServices.filter((m) => m.name.toLowerCase().startsWith(searchQuery.toLowerCase()));
		else if (selectedCategorie && selectedCategorie._id) {
			filtered = allServices.filter((m) => m.categorie._id === selectedCategorie._id);
		}
