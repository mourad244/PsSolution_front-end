...
import { getServiceCategorie, saveServiceCategorie } from '../services/serviceCategorieService';
...
class ServiceCategorieForm extends Form {
	...
	async populateServicesCategorie() {
		try {
			...
			const { data: serviceCategorie } = await getServiceCategorie(serviceCategorieId);
	...
	render() {
		...
		return (
			<div className={'card textcenter mt-3 ' + (this.props.formDisplay || serviceCategorieId ? '' : 'add-item')}>
